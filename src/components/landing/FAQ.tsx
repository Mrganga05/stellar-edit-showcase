...
export function FAQ() {
  ...
  const faqsToRender = dbFaqs && dbFaqs.length > 0
    ? dbFaqs.map(f => ({
        q: f.question,
        a: f.answer
      }))
          ...
          {isLoading && !dbFaqs ? (
            ...
            faqsToRender.map((f, i) => (
              <AccordionItem key={f.q} value={`i${i}`} className="rounded-2xl glass px-5">
                <AccordionTrigger className="py-5 text-left text-base font-medium hover:no-underline sm:text-lg">{f.q}</AccordionTrigger>
