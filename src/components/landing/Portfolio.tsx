...
function Card({ p, onOpen }: { p: Project; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      onMouseLeave={() => { setHover(false); videoRef.current?.pause(); if (videoRef.current) videoRef.current.currentTime = 0; }}
    ...
    >
      <img src={p.thumb} alt={p.title} loading="lazy" width={1280} height={768}
        className={cn("absolute inset-0 size-full object-cover transition-opacity duration-500", hover && "opacity-0")} />
      <video
        ref={videoRef} src={p.video} muted loop playsInline preload="none"
        className={cn("absolute inset-0 size-full object-cover transition-opacity duration-500", hover ? "opacity-100" : "opacity-0")}
      />
...
export function Portfolio() {
  ...
  const projectsToRender = dbProjects && dbProjects.length > 0
    ? dbProjects.map(p => ({
        id: p.id,
        title: p.title,
        category: p.category,
  ...
  return (
    <section id="work" className="relative mx-auto max-w-7xl px-5 py-28 sm:px-8 sm:py-36">
      <SectionHeading
        ...
        title={<>Cuts that <span className="text-gradient-brand">move metrics</span>.</>}
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {isLoading && !dbProjects ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="aspect-[9/16] w-full animate-pulse rounded-2xl bg-surface/50 border border-white/5" />
          ))
        ) : (
        ...
        {open && (
          ...
          >
            ...
            >
              ...
              <video src={open.video} controls autoPlay className="aspect-[9/16] max-h-[55vh] w-full bg-black object-cover" />
              <div className="space-y-6 overflow-y-auto p-6">
                <div>
                  <span className="text-[10px] uppercase tracking-[0.2em] text-electric">{open.category}</span>
                  <h3 className="mt-2 font-display text-2xl leading-tight">{open.title}</h3>
                  <p className="mt-2.5 text-xs text-muted-foreground leading-relaxed">{open.overview}</p>
                </div>
  ...
  return (
    <div>
      ...
      <ul className="mt-2 space-y-1.5 text-sm">
        {items.map((it) => (
          <li key={it} className="flex gap-2"><span className="mt-2 size-1 shrink-0 rounded-full bg-electric" />{it}</li>
        ))}
      </ul>
    </div>
  );
