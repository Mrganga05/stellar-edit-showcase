...
const mockServices = [
  { icon: Youtube, title: "YouTube Editing", desc: "Long-form retention-first edits that grow channels." },
  { icon: Smartphone, title: "Shorts Editing", desc: "Hook-first vertical edits engineered to loop." },
  { icon: Instagram, title: "Reels Editing", desc: "Editorial reels with kinetic motion and grade." },
  ...
  { icon: Megaphone, title: "Commercial Ads", desc: "30s spots built to convert on every platform." },
  { icon: Hash, title: "Social Content", desc: "TikTok, X, LinkedIn — native edits, native results." },
];

export function Services() {
  const { data: dbServices, isLoading } = useServices();

  const servicesToRender = dbServices && dbServices.length > 0
    ? dbServices.map(s => ({
  ...
  return (
    <section id="services" className="relative mx-auto max-w-7xl px-5 py-28 sm:px-8 sm:py-36">
      <SectionHeading
        eyebrow="Services"
        title={<>One editor. <span className="text-gradient-brand">Every surface</span>.</>}
      ...
      <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {isLoading && !dbServices ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-44 animate-pulse rounded-2xl bg-surface/50 border border-white/5" />
          ))
        ) : (
