...
export function Testimonials() {
  ...
  const testimonialsToRender = dbTestimonials && dbTestimonials.length > 0
    ...
    <section className="relative mx-auto max-w-7xl px-5 py-28 sm:px-8 sm:py-36">
      <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
        <SectionHeading
          ...
          title={<>Loved by <span className="text-gradient-brand">serious creators</span>.</>}
        />
        <div className="flex gap-2">
          <button onClick={prev} className="grid size-11 place-items-center rounded-full glass hover:bg-white/10" aria-label="Previous"><ArrowLeft className="size-4" /></button>
          <button onClick={next} className="grid size-11 place-items-center rounded-full glass hover:bg-white/10" aria-label="Next"><ArrowRight className="size-4" /></button>
        </div>
      </div>
        ...
        <div className="flex gap-5">
          {isLoading && !dbTestimonials ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="min-w-0 flex-[0_0_100%] sm:flex-[0_0_60%] lg:flex-[0_0_40%]">
                <div className="h-64 animate-pulse rounded-3xl bg-surface/50 border border-white/5" />
              </div>
            ))
          ) : (
            testimonialsToRender.map((t) => (
              <div key={t.name} className="min-w-0 flex-[0_0_100%] sm:flex-[0_0_60%] lg:flex-[0_0_40%]">
                <div className="flex h-full flex-col rounded-3xl glass-strong p-7">
                  ...
                  <div className="mt-auto flex items-center gap-4 pt-8">
                    <img src={t.avatar} alt={t.name} loading="lazy" width={56} height={56} className="size-14 rounded-full object-cover" />
