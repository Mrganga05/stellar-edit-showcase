...
}: { eyebrow?: string; title: ReactNode; subtitle?: ReactNode; align?: "center" | "left"; className?: string }) {
  return (
    <motion.div
    ...
    >
      {eyebrow && (
        <div className={cn("mb-5 inline-flex items-center gap-2 rounded-full glass px-3 py-1 text-xs uppercase tracking-[0.2em] text-muted-foreground", align === "center" && "mx-auto")}>
...
export function GlassCard({ className, children, glow }: { className?: string; children: ReactNode; glow?: "blue" | "violet" }) {
  return (
    <div className={cn("glass rounded-2xl", glow === "blue" && "glow-blue", glow === "violet" && "glow-violet", className)}>
      {children}
    </div>
  );
}
...
export function AnimatedCounter({ to, suffix = "", duration = 2 }: { to: number; suffix?: string; duration?: number }) {
